...
public final class Utf8 {
    ...
    public static boolean isValidUtf8(byte[] bytes) {
        return processor.isValidUtf8(bytes, COMPLETE, bytes.length);
    }
    ...
    public static int incompleteStateFor(int byte1) {
        return byte1 > -12 ? MALFORMED : byte1;
    }
    ...
    public static int incompleteStateFor(int byte1, int byte2, int byte3) {
        ...
        return ((byte2 << 8) ^ byte1) ^ (byte3 << UNSAFE_COUNT_ASCII_THRESHOLD);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static int incompleteStateFor(byte[] bytes, int index, int limit) {
        int byte1 = bytes[index + MALFORMED];
        switch (limit - index) {
            case COMPLETE /* 0 */:
    ...
    public static abstract class Processor {
        ...
        final boolean isValidUtf8(byte[] bytes, int index, int limit) {
            return partialIsValidUtf8(Utf8.COMPLETE, bytes, index, limit) == 0;
        }

        final boolean isValidUtf8(ByteBuffer buffer, int index, int limit) {
            return partialIsValidUtf8(Utf8.COMPLETE, buffer, index, limit) == 0;
        }
            ...
            if (state != 0) {
                ...
                if (byte1 < -32) {
                    ...
                    if (byte22 == 0) {
                        ...
                        byte3 = (byte) (state >> Utf8.UNSAFE_COUNT_ASCII_THRESHOLD);
                    }
                    if (byte3 == 0) {
                        int index6 = index + 1;
        ...
        private static int partialIsValidUtf8(ByteBuffer buffer, int index, int limit) {
            ...
            while (index2 < limit) {
                ...
                if (byte1 >= 0) {
                    ...
                    if (index3 >= limit + Utf8.MALFORMED) {
        ...
        final String decodeUtf8Default(ByteBuffer buffer, int index, int size) throws InvalidProtocolBufferException {
            ...
            while (offset < limit) {
                ...
                if (DecodeUtil.isOneByte(byte1)) {
                    ...
                    if (offset2 >= limit + Utf8.MALFORMED) {
                        throw InvalidProtocolBufferException.invalidUtf8();
                    }
            ...
            return new String(resultArr, (int) Utf8.COMPLETE, resultPos2);
        }
    ...
    static final class SafeProcessor extends Processor {
        ...
        */
        int encodeUtf8(CharSequence in, byte[] out, int offset, int length) {
            ...
            while (i < utf16Length) {
                ...
                if (c2 < 128 && j < limit) {
                    ...
                    throw new UnpairedSurrogateException(i + Utf8.MALFORMED, utf16Length);
        ...
        private static int partialIsValidUtf8(byte[] bytes, int index, int limit) {
            ...
            return index >= limit ? Utf8.COMPLETE : partialIsValidUtf8NonAscii(bytes, index, limit);
        }
        ...
        private static int partialIsValidUtf8NonAscii(byte[] bytes, int index, int limit) {
            while (index < limit) {
                ...
                if (byte1 >= 0) {
                    ...
                    if (index2 >= limit + Utf8.MALFORMED) {
        ...
        */
        ...
        int encodeUtf8(CharSequence in, byte[] out, int offset, int length) {
            ...
            if (inLimit > length || out.length - length < offset) {
                throw new ArrayIndexOutOfBoundsException("Failed writing " + in.charAt(inLimit + Utf8.MALFORMED) + " at index " + (offset + length));
            }
            int inIx = Utf8.COMPLETE;
            while (true) {
                c = 128;
            ...
            while (inIx < inLimit) {
                ...
                if (c3 < c && outIx2 < outLimit) {
                    ...
                    throw new UnpairedSurrogateException(inIx + Utf8.MALFORMED, inLimit);
                }
                inIx++;
                outLimit = outIx;
                j = 1;
        ...
        void encodeUtf8Direct(CharSequence in, ByteBuffer out) {
            ...
            if (inLimit > outLimit - outIx2) {
                throw new ArrayIndexOutOfBoundsException("Failed writing " + in.charAt(inLimit + Utf8.MALFORMED) + " at index " + out.limit());
            }
            int inIx = Utf8.COMPLETE;
            while (true) {
                c = 128;
        ...
        private static int unsafeEstimateConsecutiveAscii(byte[] bytes, long offset, int maxChars) {
            if (maxChars < Utf8.UNSAFE_COUNT_ASCII_THRESHOLD) {
                return Utf8.COMPLETE;
            }
        ...
        private static int partialIsValidUtf8(byte[] bytes, long offset, int remaining) {
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    remaining2 += Utf8.MALFORMED;
                    offset2 = offset3;
                }
                if (remaining2 == 0) {
                    return Utf8.COMPLETE;
                }
                int remaining3 = remaining2 + Utf8.MALFORMED;
                if (byte1 < -32) {
                    if (remaining3 == 0) {
                        return byte1;
                    }
                    remaining2 = remaining3 + Utf8.MALFORMED;
                    if (byte1 < -62) {
                        break;
                    }
