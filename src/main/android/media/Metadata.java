...
public class Metadata {
    ...
    private boolean scanAllRecords(Parcel parcel, int bytesLeft) {
        ...
        while (true) {
            ...
            if (size <= 12) {
                Log.m110e(TAG, "Record is too short");
                error = true;
                break;
            }
            ...
            if (!checkMetadataId(metadataId)) {
                ...
                Log.m110e(TAG, "Duplicate metadata ID found");
                error = true;
                break;
            } else {
                ...
                try {
                    ...
                    Log.m110e(TAG, "Invalid size: " + e.getMessage());
                    error = true;
                }
            }
        }
        Log.m110e(TAG, "Invalid metadata type " + metadataType);
        error = true;
        if (bytesLeft != 0 || error) {
            Log.m110e(TAG, "Ran out of data or error on record " + recCount);
            this.mKeyToPosMap.clear();
            return false;
        }
        return true;
    ...
    public boolean parse(Parcel parcel) {
        if (parcel.dataAvail() < 8) {
            Log.m110e(TAG, "Not enough data " + parcel.dataAvail());
            return false;
        }
        int pin = parcel.dataPosition();
        int size = parcel.readInt();
        if (parcel.dataAvail() + 4 < size || size < 8) {
            Log.m110e(TAG, "Bad size " + size + " avail " + parcel.dataAvail() + " position " + pin);
            parcel.setDataPosition(pin);
            return false;
        }
        int kShouldBeMetaMarker = parcel.readInt();
        if (kShouldBeMetaMarker != kMetaMarker) {
            Log.m110e(TAG, "Marker missing " + Integer.toHexString(kShouldBeMetaMarker));
            parcel.setDataPosition(pin);
            return false;
    ...
    private boolean checkMetadataId(int val) {
        if (val <= 0 || (31 < val && val < 8192)) {
            Log.m110e(TAG, "Invalid metadata ID " + val);
            return false;
        }
        return true;
    }
