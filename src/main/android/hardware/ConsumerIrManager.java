...
public final class ConsumerIrManager {
    ...
    public boolean hasIrEmitter() {
        ...
        if (iConsumerIrService == null) {
            Log.m104w(TAG, "no consumer ir service.");
            return false;
        }
        try {
    ...
    public void transmit(int carrierFrequency, int[] pattern) {
        IConsumerIrService iConsumerIrService = this.mService;
        if (iConsumerIrService == null) {
            Log.m104w(TAG, "failed to transmit; no consumer ir service.");
            return;
        }
        try {
    ...
    public CarrierFrequencyRange[] getCarrierFrequencies() {
        ...
        try {
            ...
            if (freqs.length % 2 != 0) {
                Log.m104w(TAG, "consumer ir service returned an uneven number of frequencies.");
                return null;
            }
