...
public final class ContentCaptureServiceInfo {
    ...
    private ContentCaptureServiceInfo(Context context, ServiceInfo si) {
        if (!Manifest.C0000permission.BIND_CONTENT_CAPTURE_SERVICE.equals(si.permission)) {
            Slog.m90w(TAG, "ContentCaptureService from '" + si.packageName + "' does not require permission " + Manifest.C0000permission.BIND_CONTENT_CAPTURE_SERVICE);
        ...
        try {
            ...
            if (XML_TAG_SERVICE.equals(parser.getName())) {
                ...
                TypedArray afsAttributes = resources.obtainAttributes(allAttributes, C4057R.styleable.ContentCaptureService);
                settingsActivity = afsAttributes.getString(0);
                if (afsAttributes != null) {
                    afsAttributes.recycle();
                }
            } else {
                Log.m110e(TAG, "Meta-data does not start with content-capture-service tag");
            }
